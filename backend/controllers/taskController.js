...
export const updateTaskCheckList = async(req,res)=>{
    try {
        ...
        //auto update progress nased on checklist
        const completedCount = task.todoChecklist.filter((item)=>item.completed).length

        const totalItems = task.todoChecklist.length
        task.progress = totalItems>0?Math.round((completedCount/totalItems)*100):0

        //auto-marks task ascommpleted if all items are checked
...
export const getDashboardData = async(req,res)=>{
    try {
       ...
       //ensure all possible statue include
...
export const getUserDashboardData = async(req,res)=>{
    try {
        const userId = req.user._id //only fetch data for loggein user

            const totalTasks = await Task.countDocuments({assignedTo:userId})
            const pendingTasks = await Task.countDocuments({assignedTo:userId,status:"Pending"})
            const completedTasks = await Task.countDocuments({assignedTo:userId,status:"Cmpleted"})
            const overdueTasks = await Task.countDocuments({
                assignedTo:userId,
                status:{$ne:"Completed"},
            dueDate:{$lt:new Date()}
